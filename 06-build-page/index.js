...
function copyDirectory(directoryPath, copyDirectoryPath) {
  if (!directoryPath || !copyDirectoryPath) {
    return;
  }

  fs.mkdir(copyDirectoryPath, {recursive: true}, () => {
   let readCopy = fs.promises.readdir(directoryPath, {withFileTypes: true})
...
async function create() {
  ...
  await cleanDirectory(projectDist);
  copyDirectory(path.join(__dirname, 'assets'), path.join(projectDist, 'assets'));
  createStyle(path.join(__dirname, 'styles'), path.join(projectDist, 'style.css'));
  await createHTML(path.join(__dirname, 'template.html'), path.join(__dirname, 'components'), path.join(projectDist, 'index.html'));
}

function createStyle(sourcePath, resultPath) {
  let style = [];
