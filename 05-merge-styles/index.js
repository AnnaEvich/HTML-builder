...
async function readFiles() {
  fs.unlink(path.join(__dirname, 'project-dist', 'bundle.css'), () => {
    let readFileInfo = fs.promises.readdir(path.join(__dirname, 'styles'), {withFileTypes: true});

    readFileInfo.then(files => {
      for (let elem of files) {
        let mainPath = path.join(path.join(__dirname, 'styles', elem['name']));
        if (path.join(__dirname, "styles", path.basename(path.join(__dirname, "styles", mainPath))).includes(".css")) {
          let doneRead = fs.createReadStream(mainPath, 'utf-8');
